...
onMounted(async () => {
    ...
    try {
        ...
        studentPassword.value = res.data[0].password
        studentPhoneNumber.value = res.data[0].phone_number
        studentAddress.value = res.data[0].student_address
    } catch (error) {
        ElMessage.error(`服务器获取学生数据失败:${error}`)
        console.log("服务器获取学生数据失败:", error);
    }
});
...
const submitForm = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate((valid) => {
        if (valid) {
            ...
            if (disableItems.password) {
                updatePass = studentPassword.value;
            } else {
                updatePass = ruleForm.pass;
                ifUpdate = true;
            }
            if (disableItems.phone) {
                updatePhone = studentPhoneNumber.value;
            } else {
                updatePhone = ruleForm.pass;
                ifUpdate = true;
            }
            if (disableItems.address) {
                updateAddress = studentAddress.value;
            } else {
            ...
            if (ifUpdate) {
                try {
                    updateStudentData(updatePass, updatePhone, updateAddress, sessionStorage.getItem("number"));
                    ElMessage.success('更新成功！')
                    disableItems.password = true;
                    disableItems.phone = true;
                    disableItems.address = true;
                } catch (error) {
                    ElMessage.error(`服务器更新学生数据失败:${error}`)
                    console.log("服务器更新学生数据失败:", error);
                }
            }
        } else {
...
<template>
    <el-form ref="ruleFormRef" style="max-width: 600px" :model="ruleForm" :rules="rules" label-width="auto"
        ...
        <el-form-item label="姓名">
            <el-input disabled :placeholder="studentName"/>
        </el-form-item>
        ...
        <el-form-item label="电话" prop="phone">
            <el-input v-model="ruleForm.phone" :placeholder="studentPhoneNumber" :disabled="disableItems.phone">
                <template #append>
        ...
        <el-form-item label="地址" prop="address">
            <el-input v-model="ruleForm.address" :placeholder="studentAddress" :disabled="disableItems.address">
                <template #append>
