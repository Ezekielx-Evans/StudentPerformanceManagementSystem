...
export const getClassWeekData = async (studentId, subjectDay, subjectTerm) => {
    try {
        return await api.get(`/classWeekData?id=${studentId}&day=${subjectDay}&term=${subjectTerm}`);
    } catch (error) {
        if (error.response) {
            // 请求成功发出且服务器也响应了状态码，但状态代码超出了 2xx 的范围
            console.log(error.response.data);
...
export const insertStudentData = async (studentName, studentId, studentGender) => {
    try {
        await api.put(`/insertStudent`, {
